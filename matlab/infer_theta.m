function [p_vec,tot_samples,nll_exact,theta_inf,output_vec]=infer_theta(model,method,stim,resp,settings)
...
%         Nreps = Nsamples;
        ...
        Ntrials = size(stim,1);
        thresh = settings.thresh*Ntrials;
        [nll_fix,~,output_fix] = estimate_nll_fixed1(model,stim,resp,x0,Nreps_fixed); % use x0 to get vector p by calling fixed sampling
...
if strcmp(submethod,'ibs') || strcmp(submethod,'ibs_alloc') || strcmp(submethod,'ibs_dynamic') || strcmp(submethod,'fixed') || strcmp(submethod,'fixedb')
...
elseif strcmp(submethod,'ibs_alloc') 
    p_vec = p_vec;
else
    p_vec = 0;
end 
