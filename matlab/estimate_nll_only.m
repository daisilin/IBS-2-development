function [nll,nll_var,output]=estimate_nll_only(model,stim,resp_real,theta,thresh,highrep,Nreps)
...
for iRep = 1:Nreps_max 
    ...
    while n>0 && nll_sum<thresh 
        resp(ind1) = feval(['generate_resp_' model],stim,theta,ind1); %simulate responses from the model for active trials && no-hit trials only; 
